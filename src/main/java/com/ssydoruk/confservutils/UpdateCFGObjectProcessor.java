...
public class UpdateCFGObjectProcessor {
	...
	private Message commitUpdateKVP(CfgObject obj) throws ProtocolException {
		if (!updateSections.isEmpty() || !createSections.isEmpty() || !deleteSections.isEmpty()
			...
			ConfObject obj1 = (ConfObject) d
				.getOrCreatePropertyValue(metaData
					.getCfgClass(className)
					.getDelta()
					.getClassDescription()
					.getAttributeByName(className)
					.getSchemaName());
...
//                obj1.setPropertyValue("commandLine", "aaa");
	...
	private void fillUpdateKVP(IUpdateSettings us, CfgObject obj, KeyValueCollection kv) {
		...
		case ADD_OPTION_FORCE: {
			...
			if (addedKVP != null) {
				for (UserProperties userProperties : addedKVP) {
					addAddKeyForce(userProperties.getSection(), userProperties.getKey(), userProperties
						.getValue(), obj);
...
//                    }
			...
			for (Object object : kv) {
				if (object instanceof KeyValuePair) {
					...
					if (valueType == ValueType.TKV_LIST) {
						for (Object _kvInstance : (KeyValueCollection) value) {
							KeyValuePair kvInstance = (KeyValuePair) _kvInstance;
							addUpdateKey(section, kvInstance.getStringKey(), us
								.KVPreplaceWith(kvInstance.getStringValue()));
							if (us.isMakeBackup()) {
	...
	public Message updateObj(IUpdateSettings us, CfgObject obj, KeyValueCollection kv, CfgObject newObjectProperties)
	...
	private String getCurValue(CfgObject obj, String section, String origProperty) {
		KeyValueCollection property = (customKVPProc == null) ? (KeyValueCollection) obj.getProperty("userProperties")
				: customKVPProc.getCustomKVP(obj);
					...
					if (valueType == ValueType.TKV_LIST) {
						for (Object _kvInstance : (KeyValueCollection) value) {
							...
							if (kvInstance.getStringKey().startsWith(BACKUP_PREFIX)) {
								ret
									.add(new UserProperties(kvp.getStringKey(), kvInstance.getStringKey(), kvInstance
										.getStringValue()));
							}
						}
					} else {
...
//                        + ": " + ex.getMessage());
				...
				theForm
					.requestOutput("Not able to delete object " + getObjDescr(deleteObject) + ": " + ex.getMessage());
			}
		}
